...
def train(model, dataset, loss_model, optimizer, train_dataset, test_dataset, epochs, start_epoch=1):
    ...
    for epoch in range(start_epoch, epochs+1):
        ...
        print(f"({get_time()}) Epoch {epoch} | PSNR: {avg_psnr:.2f} | SSIM: {avg_ssim:.3f} | loss={loss:.6f}")

        if avg_psnr > best_psnr:
            best_psnr, best_ssim = avg_psnr, avg_ssim
            model_name = f"net_psnr_{best_psnr:.2f}_ssim_{best_ssim:.3f}_epoch_{epoch}_dataset_{dataset}.h5"
